...
class Scale():
    ...
    def standard_competition_ranking(sequence):
        ...
        ranking = map(lambda x: index[order[x] - 1], range(len(sequence)))
        return ranking
    ...
    def fractional_ranking(sequence):
        ...
        for i in range(1, len(sequence)):
            if sequence[i] == sequence[i - 1]:
                len_same_ranking += 1
                if i == len(sequence) - 1:
                    value_same_ranking = reduce(lambda x, y: x + y, range(i - len_same_ranking + 2, i + 2))*1.0/len_same_ranking
                    for j in range(i - len_same_ranking + 1, i + 1):
                        index[j] = value_same_ranking
            else:
                value_same_ranking = reduce(lambda x, y: x + y, xrange(i - len_same_ranking + 1, i + 1))*1.0/len_same_ranking
                for j in range(i - len_same_ranking, i):
                    index[j] = value_same_ranking
    ...
    def percentile(sequence):
        '''
        Examples:
            >>> Scale.percentile([10, 5, 20])
            [50.0, 25.0, 100.0]
        '''
        max_sequence = max(sequence)
        return map(lambda x: x*100.0/max_sequence, sequence)

    @staticmethod
    def minmax(sequence):
        '''
        Examples:
            >>> Scale.minmax([2, 1, 3, 4])
            (1, 4)
        '''
